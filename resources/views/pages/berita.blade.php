...
@section('title', 'Berita - UPTD Puskesmas Pantai Amal')
    ...
    <div class="mx-auto max-w-7xl px-6 py-12">
                ...
                @foreach ($beritas as $berita)
                    <div x-data x-intersect.once="$el.classList.add('reveal-visible')" class="reveal" style="transition-delay: {{ $loop->index * 80 }}ms">
                        <x-content-card
                            ...
                            :date="$berita->published_at?->format('d M Y')"
                            :category="$berita->category?->name"
                        />
                    </div>
                @endforeach
            </div>
        ...
        @else
            <div class="py-20 text-center">
                ...
                <p class="text-sm text-neutral-400">Belum ada berita yang dipublikasikan.</p>
            </div>
        @endif
    </div>
</div>
